...
prop_compose! {
    pub fn message_strat()
        ...
        let real_msg_type = match &payload {
            ...
            PayloadContent::NetworkTrace(_) => todo!()
        };
...
fn vec_from_length(length: u8) -> impl Strategy<Value = Vec<HashSet<u8>>> {
    ...
    for index in 1..length {
        let inner = hash_set(0..index, 0..K); // ensure unique elements
        result.push(inner);
    }
    result
}

fn vecs_from_sets(input: Vec<HashSet<u8>>) -> Vec<Vec<u8>> {
    let mut output = vec![];
    for inner in input {
